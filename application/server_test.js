...
app.get('/dnaenroll', (req, res) => {
    res.render('dnaenroll.html')
})

app.post('/dnaenroll', async (req, res) => {

    try {
        data= {
            userKey: "1",
            aging: req.body.aging,
            elasticity: req.body.elasticity,
            Pigmentation: req.body.pigmentation
        }
         // Create a new file system based wallet for managing identities.
         const walletPath = path.join(process.cwd(), 'wallet');
         const wallet = new FileSystemWallet(walletPath);
         ...
         // Get the contract from the network.
         const contract = network.getContract('genedata');
 
         // Submit the specified transaction.
         // createCar transaction - requires 5 argument, ex: ('createCar', 'CAR12', 'Honda', 'Accord', 'Black', 'Tom')
         // changeCarOwner transaction - requires 2 args , ex: ('changeCarOwner', 'CAR10', 'Dave')
 //        await contract.submitTransaction('createCar', 'CAR11', 'Hnda', 'Aord', 'Bla', 'Tom');
         await contract.submitTransaction('addGene', data.userKey, data.aging,data.elasticity,data.Pigmentation);
         console.log('Transaction has been submitted');
